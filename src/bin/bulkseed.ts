 ...
 * from the catalog (real names → looks legit), then bulk-loads them. Each tool
 * gets a random reliability profile, so the leaderboard and feed show a natural
 * spread. Used for demo/staging data, not the public submit path.
 */
...
const TARGET = values.count ? Math.max(1, Number.parseInt(values.count, 10)) : 1500;
const N_TOOLS = values.tools ? Math.max(1, Number.parseInt(values.tools, 10)) : 160;

// Deterministic RNG so a reseed is reproducible.
...
// Reliability tiers → a legit spread from excellent to broken.
const TIERS = [
  { w: 0.28, rel: [0.9, 0.98], latMed: [250, 1500] }, // excellent
  { w: 0.3, rel: [0.75, 0.9], latMed: [500, 3000] }, // good
  { w: 0.27, rel: [0.55, 0.75], latMed: [800, 5000] }, // mediocre
  { w: 0.15, rel: [0.3, 0.55], latMed: [1000, 9000] }, // flaky/broken
];
function pickTier() {
  let r = rng();
...
function taskKind(category: string): string {
  return pick(TASK_BY_CATEGORY[category] ?? GENERIC_TASKS);
}
...
async function main() {
  ...
  console.log(`bulkseed → ${backend}: target ${TARGET} reviews across ~${N_TOOLS} real tools`);

  // Make sure there's a real catalog to review.
  let catalog = await db.countTools();
  if (catalog < N_TOOLS * 2 && !values['no-import']) {
    console.log('catalog is thin — importing real tools from the MCP registry + npm…');
    await importMcpRegistry(db, { max: 6000, log: (m) => process.stdout.write(`\r${m}   `) }).catch(
      (e) => console.warn('\nregistry import: ' + e.message),
    );
    await importNpm(db, { max: 2500, log: (m) => process.stdout.write(`\r${m}   `) }).catch(
      (e) => console.warn('\nnpm import: ' + e.message),
    );
    catalog = await db.countTools();
    console.log(`\ncatalog now ${catalog} tools`);
  }

  const tools: ToolRecord[] = await db.sampleTools(N_TOOLS);
  if (tools.length === 0) {
    console.error('no tools to review — import a catalog first (filterly-import).');
    process.exit(1);
  }

  // A small fleet of mature reporters (probes + agents). Every review is signed
  // by one of these; because each ends up with 100+ verified outcomes, the
  // maturity weighting treats them as established, so scores aren't damped.
  ...
  // Assign each tool a profile and a review budget, filling up to TARGET.
  const now = Date.now();
  ...
  const toolProfiles = tools.map((t) => {
    ...
    return {
      ...
      // recent regression on ~8% of tools, for a believable "declining" signal
  ...
  while (rows.length < TARGET) {
    ...
    for (let k = 0; k < perTool && rows.length < TARGET; k++) {
      ...
      const verified = rng() < 0.85; // most are signed; some anonymous
      const note =
        rng() < 0.16
          ? status === 'failure'
  ...
  console.log(`generating done — bulk-inserting ${rows.length} reviews…`);
  await db.bulkInsertOutcomes(rows);

  const total = await db.countOutcomes();
  const good = rows.filter((r) => r.outcome.status === 'success').length;
  const bad = rows.filter((r) => r.outcome.status === 'failure').length;
  const partial = rows.length - good - bad;
  console.log(
    `done: inserted ${rows.length} reviews (${good} success / ${partial} partial / ${bad} failure) ` +
      `across ${new Set(rows.map((r) => r.outcome.tool_id)).size} tools. ` +
      `Total outcomes now ${total}, catalog ${await db.countTools()} tools.`,
  );
  await db.close();
}

function shuffledFailureWeights(): Record<string, number> {
  const out: Record<string, number> = {};
  for (const m of FAILURE_MODES) out[m] = rng() < 0.5 ? 0 : 1 + Math.floor(rng() * 4);
  // guarantee at least one mode has weight
  if (Object.values(out).every((v) => v === 0)) out.timeout = 2;
  return out;
}
function pickWeighted(weights: Record<string, number>): StoredOutcome['failure_mode'] {
